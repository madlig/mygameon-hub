...
export async function listAllFiles(drive, folderId) {
  ...
  // Deduplicate by targetId to prevent React key errors and redundant checks
  const uniqueItems = []
...
export function aggregateByOwner(fileResults, ownerMap) {
  ...
  for (const res of fileResults) {
    ...
    workspacesMap[email].allFiles.push({
      id: res.targetId,
      name: res.name,
      status: res.status,
      reason: res.reason
  ...
  const sortedWorkspaces = workspaces.sort((a, b) => a.email.localeCompare(b.email, undefined, { numeric: true }))

  return { limited, workspaces: sortedWorkspaces, debugErrors }
}
