  ...
  do {
    ...
    for (const f of files) {
      const targetId = f.mimeType === 'application/vnd.google-apps.shortcut' && f.shortcutDetails
        ...
        : f.id
      if (targetId) {
        allItems.push({ targetId, name: f.name, id: f.id })
      }
    }
...
export function classifyError(err) {
  ...
  if (reasonLower.includes('downloadquota') || reasonLower.includes('quotaexceeded')) {
      ...
      while (attempts <= maxRetries) {
        try {
          await drive.files.get(
            { fileId: item.targetId, alt: 'media' },
          ...
          return { ...item, status: type, reason }
        }
      }
    })
 ...
 * @returns { limited: [], workspaces: [] }
 */
export function aggregateByOwner(fileResults, ownerMap) {
  const workspacesMap = {} // email -> { totalFiles, limitedFiles, files: [] }
  ...
  for (const res of fileResults) {
    ...
    if (!workspacesMap[email]) {
      workspacesMap[email] = {
        ...
        limitedFiles: 0,
        files: [],
        reason: '' // stores the last limit reason
      }
    }
