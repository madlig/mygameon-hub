...
export async function syncWorkspaceCatalog(email) {
  ...
  const parentId = account.gameFolderId || 'root';

  // 1. List semua FOLDER di root My Drive (= folder game)
  const gameFolders = [];
  let pageToken = null;
  try {
    do {
    ...
    return { added: 0, updated: 0, removed: 0, error: `Gagal membaca Drive ${email}: ${e.message}` };
  }
