...
export async function startJob(folderPath, targetEmail, config) {
  ...
  try {
     ...
     if (config.autoDelete) {
         updateState({}, `Mode Auto-Uninstall/Delete aktif. Membersihkan file lokal...`)
         try {
             const uninsInno = path.join(folderPath, 'unins000.exe')
             const uninsGeneric = path.join(folderPath, 'uninstall.exe')
             
             if (fs.existsSync(uninsInno)) {
                 updateState({}, `Uninstaller InnoSetup ditemukan. Menjalankan silent uninstall...`)
                 try {
                     await new Promise((resolve, reject) => {
                         const child = spawn(uninsInno, ['/VERYSILENT', '/SUPPRESSMSGBOXES', '/NORESTART'])
                         child.on('close', resolve)
                         child.on('error', reject)
                     })
                     updateState({}, `Proses uninstall InnoSetup selesai.`)
                 } catch (e) {
                     updateState({}, `Gagal menjalankan uninstaller. Fallback ke hapus manual.`)
                     fs.rmSync(folderPath, { recursive: true, force: true })
                 }
             } else if (fs.existsSync(uninsGeneric)) {
                 updateState({}, `Uninstaller generic ditemukan. Menjalankan silent uninstall...`)
                 try {
                     await new Promise((resolve, reject) => {
                         const child = spawn(uninsGeneric, ['/S'])
                         child.on('close', resolve)
                         child.on('error', reject)
                     })
                     updateState({}, `Proses uninstall generic selesai.`)
                 } catch (e) {
                     updateState({}, `Gagal menjalankan uninstaller. Fallback ke hapus manual.`)
                     fs.rmSync(folderPath, { recursive: true, force: true })
                 }
             } else {
                 // Hapus folder asli manual jika tidak ada uninstaller
                 fs.rmSync(folderPath, { recursive: true, force: true })
                 updateState({}, `Uninstaller tidak ditemukan. Folder asli dihapus paksa: ${folderName}`)
             }
             
             // Hapus file rar part
             for (const partName of parts) {
                 if (fs.existsSync(path.join(dir, partName))) {
