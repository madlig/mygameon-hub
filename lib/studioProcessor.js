...
export async function uploadJob(folderPath, targetEmail, config = {}, options = {}) {
  ...
  try {
    await connectToDatabase();
    const folderName = options.gameName || path.basename(folderPath).replace(/\.(rar|7z|zip)$/i, '');
    const dir = fs.statSync(folderPath).isDirectory() ? path.dirname(folderPath) : path.dirname(folderPath);
    let allFiles = fs.readdirSync(dir);

    // Filter file yang diawali nama game dan berformat .rar / part
    let parts = allFiles.filter((f) => f.startsWith(folderName) && (f.endsWith('.rar') || f.endsWith('.7z') || f.endsWith('.zip')));
    parts.sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));

    // Jika raw folder dan belum ada part RAR, jalankan kompresi otomatis terlebih dahulu
    if (parts.length === 0 && fs.existsSync(folderPath) && fs.statSync(folderPath).isDirectory()) {
      updateState({ text: `Mengompresi folder ${folderName} sebelum upload...` }, `Otomatis membuat arsip WinRAR untuk ${folderName}...`);
      await archiveJob(folderPath, config);
      allFiles = fs.readdirSync(dir);
      parts = allFiles.filter((f) => f.startsWith(folderName) && (f.endsWith('.rar') || f.endsWith('.7z') || f.endsWith('.zip')));
      parts.sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
    }
