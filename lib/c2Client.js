...
if (!fs.existsSync(envPath)) {
  envPath = path.join(process.resourcesPath, '.env.local') // In production
}
...
async function scanLocalFolders() {
  ...
  let targetPath = process.env.STUDIO_UPLOAD_DIR || 'D:\\Game\\Shopee\\GameUpload'
...
async function sendHeartbeat() {
  try {
    const folders = await scanLocalFolders()
      ...
      try {
        if (doc.type === 'START_UPLOAD') {
          ...
          try {
            ...
            currentCommandId = null // Free the lock
          }
