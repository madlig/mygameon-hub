    ...
    for (const item of items) {
      ...
      try {
        ...
        if (stats.isDirectory()) {
          ...
          folders.push({ name: item, path: fullPath, hasArchive: partsCount > 0, archiveParts: partsCount })
          ...
          if (!isSecondaryPart) {
            ...
            folders.push({ name: baseName, path: fullPath, isArchiveFile: true, hasArchive: true, archiveParts: Math.max(1, siblingParts.length) })
          }
        }
      } catch (_) {}
    }
        ...
        if (doc.type === 'START_ARCHIVE' || doc.type === 'START_UPLOAD') {
          ...
          try {
            ...
            for (let i = 0; i < Math.min(images.length, 4); i++) {
              await downloadImage(images[i], path.join(targetDir, `ss${i + 1}.jpg`))
            }
...
`
            ...
            console.log('Launching Photoshop...')
            
            exec(`${psPath} -r "${runnerPath}"`, (error) => {
              if (error) {
                console.error('Failed to run Photoshop script:', error)
              } else {
                console.log('Photoshop script finished or launched.')
              }
            })

            // Tunggu 8 detik untuk Photoshop selesai render
            await new Promise((resolve) => setTimeout(resolve, 8000))

            // Cari file preview yang baru dihasilkan
            let previewImg = ''
            const possibleFiles = ['preview.jpg', 'preview.png', 'feed.jpg', 'feed.png']
            for (const f of possibleFiles) {
              if (fs.existsSync(path.join(targetDir, f))) {
                previewImg = path.join(targetDir, f)
                break
              }
            }

            // Clean up temporary runner JSX
            try {
