...
async function startC2Client() {
  ...
  try {
    ...
    async function processCommand(doc) {
      ...
      try {
        if (doc.type === 'START_ARCHIVE' || doc.type === 'START_UPLOAD' || doc.type === 'START_EXTRACT' || doc.type === 'START_EXTRACT_AND_UPLOAD') {
          // Dynamic import to use ESM module in CommonJS electron main
          const studioProcessor = await import('./studioProcessor.js')
          
          try {
            if (doc.type === 'START_EXTRACT') {
              await studioProcessor.extractJob(doc.payload.targetFolder)
            } else if (doc.type === 'START_ARCHIVE') {
              await studioProcessor.archiveJob(
                doc.payload.targetFolder,
                doc.payload.rarConfig
              )
            } else if (doc.type === 'START_EXTRACT_AND_UPLOAD') {
              const { outputDir, cleanGameName } = await studioProcessor.extractJob(doc.payload.targetFolder)
              await studioProcessor.uploadJob(
                ...
                {
                  ...(doc.payload.options || {}),
                  gameName: cleanGameName || doc.payload.options?.gameName || null
                }
              )
